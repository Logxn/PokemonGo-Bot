    ...
    public class Session : IDisposable
    {
        ...
        public async Task CheckHasherVersion()
        {
            using (var checkHttpClient = new HttpClient())
            {
                ...
                if (result < 0)
                {
                    var str = string.Format("The version of the {0} ({1}) does not match the minimal API version of PokemonGo ({2}). Set 'Configuration.IgnoreHashVersion' to true if you want to disable the version check.",nameof(Configuration.Hasher),Configuration.Hasher.PokemonVersion,pogoVersion);
                    throw new HashVersionMismatchException(str);
                }
            }
        }
