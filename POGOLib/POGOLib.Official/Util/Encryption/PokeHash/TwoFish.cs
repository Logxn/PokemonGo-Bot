...
namespace POGOLib.Official.Util.Encryption.PokeHash
{
    public static class TwoFish
    {
        ...
        {
            int b2 = RightUShift(x, 24) & 0xFF;
            int g2 = ((b2 << 1) ^ ((b2 & 0x80) != 0 ? RS_GF_FDBK : 0)) & 0xFF;
            int g3 = RightUShift(b2, 1) ^ ((b2 & 0x01) != 0 ? RightUShift(RS_GF_FDBK, 1) : 0) ^ g2;
            int result = (x << 8) ^ (g3 << 24) ^ (g2 << 16) ^ (g3 << 8) ^ b2;
            return result;
        }

        private static int f32(int k64Cnt, int x, System.Collections.Generic.IList<int> k32)
        {
            int b0 = _b0(x);
            int b1 = _b1(x);
            int b2 = _b2(x);
        ...
        private static int fe32(System.Collections.Generic.IList<int> sBox, int x, int r)
        {
            return sBox[2 * b(x, r)]
                    ^ sBox[2 * b(x, r + 1) + 1]
