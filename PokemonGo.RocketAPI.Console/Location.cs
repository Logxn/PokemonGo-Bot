    ...
    public partial class LocationSelect : Form
    {
        ...
        private void showMap()
        {
            try
            {
                ...
                this.map.Position = new GMap.NET.PointLatLng(40.764883, -73.972967);
                this.map.MinZoom = 0;
                this.map.MaxZoom = 20;
                this.map.Zoom = 16;
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            if (this.textBox1.Text.Length > 0 && this.textBox1.Text != "-")
            {
                try
                {
                    ...
                    this.map.Position = new GMap.NET.PointLatLng(lat, this.map.Position.Lng);
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.Message);
        ...
        private void textBox2_TextChanged(object sender, EventArgs e)
        {
            if (this.textBox2.Text.Length > 0 && this.textBox2.Text != "-")
            {
                try
                {
                    ...
                    this.map.Position = new GMap.NET.PointLatLng(this.map.Position.Lat, lng);
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.Message);
