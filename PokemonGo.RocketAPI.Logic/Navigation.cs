namespace PokemonGo.RocketAPI.Logic
{
    ...
    public class Navigation
    {
        ...
        public async Task<PlayerUpdateResponse> HumanLikeWalking(GeoCoordinate targetLocation, double walkingSpeedInKilometersPerHour, Func<Task> functionExecutedWhileWalking)
        {
            var speedInMetersPerSecond = walkingSpeedInKilometersPerHour / 3.6;

            var sourceLocation = new GeoCoordinate(this._client.CurrentLat, this._client.CurrentLng);
            ...
            var result = await this._client.UpdatePlayerLocation(waypoint.Latitude, waypoint.Longitude, this._client.getSettingHandle().DefaultAltitude);
            ...
            do
            {
                ...
                sourceLocation = new GeoCoordinate(this._client.CurrentLat, this._client.CurrentLng);
                ...
                if (currentDistanceToTarget < 30)
                {
                    ...
                    {
                        Logger.ColoredConsoleWrite(ConsoleColor.DarkCyan, $"We are within 30 meters of the target. Speeding down to ~10 km/h to not pass the target.");
                ...
                result = await this._client.UpdatePlayerLocation(waypoint.Latitude, waypoint.Longitude, this._client.getSettingHandle().DefaultAltitude);
        ...
        private List<int> calcCrossing(List<int> _chromosome1, List<int> _chromosome2)
        {
            ...
            var rnd = new Random();
            var p = rnd.Next(1, child.Count - 2);
            for (; p < child.Count - 1; ++p)
            {
                for (var i = 0; i < _chromosome2.Count; ++i)
                {
        ...
        private int calcFitness(ref FortData[] pokeStops, List<int> _chromosome, double walkingSpeedInKilometersPerHour)
        {
            ...
            var time = 0.0;
            var length = 0.0;
            for (var i = 0; i < _chromosome.Count - 1; ++i)
            {
                var distance = DistanceBetween2Coordinates(pokeStops[_chromosome[i]].Latitude, pokeStops[_chromosome[i]].Longitude, pokeStops[_chromosome[i + 1]].Latitude, pokeStops[_chromosome[i + 1]].Longitude);
                if (distance <= 40)
                {
            ...
            if (this._client.getSettingHandle().navigation_option == 1)
            {
        ...
        private List<List<int>> selection(FortData[] pokeStops, List<List<int>> population, double walkingSpeedInKilometersPerHour)
        {
            ...
            for (var c = 0; c < population.Count; ++c)
            {
                var temp = this.calcFitness(ref pokeStops, population[c], walkingSpeedInKilometersPerHour);
                sumPop += temp;
                fittnes.Add(temp);
            }
            ...
            do
            {
                ...
                var tempSumPop = 0;
                for (var c = fittnesSorted.Count - 1; c > 0; --c)
                {
