...
namespace PokeMaster.Logic.Functions
{
    ...
    public static class Setout
    {
        ...
        private static void EvolveAllPokemonWithEnoughCandy(IEnumerable<PokemonId> filter )
        {
            ...
            var pokemonToEvolve = Setout.GetPokemonToEvolve(true,filter);
        ...
        private static void StatsLog(Client client)
        {
            ...
            var pokemonToEvolve = (Setout.GetPokemonToEvolve()).Count();
        ...
        {
            var stats = Setout.GetPlayerStats().First();
        ...
        private static void TransferDuplicatePokemon(bool keepPokemonsThatCanEvolve = false, bool transferFirstLowIv = false)
        {
            ...
            if (GlobalVars.TransferDoublePokemons)
            {
                ...
                var duplicatePokemons = Setout.GetDuplicatePokemonToTransfer(GlobalVars.HoldMaxDoublePokemons, keepPokemonsThatCanEvolve, transferFirstLowIv);
                ...
                foreach (var duplicatePokemon in duplicatePokemons)
                {
                    ...
                    if (!GlobalVars.pokemonsToHold.Contains(duplicatePokemon.PokemonId))
                    {
                        ...
                        var bestPokemonOfType = Setout.GetHighestCPofType(duplicatePokemon);
                        var bestPokemonsCpOfType = Setout.GetHighestCPofType2(duplicatePokemon);
                        var bestPokemonsIvOfType = Setout.GetHighestIVofType(duplicatePokemon);
        ...
        {
            var inv =  Logic.objClient.Inventory.GetInventory().Result;
