namespace PokemonGo.RocketAPI.Logic
{
    ...
    public class Logic
    {
        ...
        public async Task Execute()
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(this._clientSettings.TelegramAPIToken) && !string.IsNullOrEmpty(this._clientSettings.TelegramName))
                    {
                        try
                        {
        ...
        public async Task PostLoginExecute()
        {
            while (true)
            {
                try
                {
                    ...
                    await this._inventory.ExportPokemonToCSV(profil.Profile);
                    ...
                    await this.TransferDuplicatePokemon(this._clientSettings.keepPokemonsThatCanEvolve);
            ...
            foreach (var pokemon in pokemons)
            {
                ...
                if (encounterPokemonResponse.Status == EncounterResponse.Types.Status.EncounterSuccess)
                {
                    ...
                    if (caughtPokemonResponse.Status == CatchPokemonResponse.Types.CatchStatus.CatchSuccess)
                    {
                        ...
                        Logger.ColoredConsoleWrite(ConsoleColor.Magenta, $"We caught a {StringUtils.getPokemonNameByLanguage(this._clientSettings, pokemon.PokemonId)} with CP {encounterPokemonResponse?.WildPokemon?.PokemonData?.Cp} ({PokemonInfo.CalculatePokemonPerfection(encounterPokemonResponse?.WildPokemon.PokemonData)}% perfect) using a {bestPokeball} and we got {caughtPokemonResponse.Scores.Xp.Sum()} XP.");
        ...
        private async Task ExecuteFarmingPokestopsAndPokemons(Client client)
        {
            ...
            var mapObjects = await this._client.GetMapObjects();
            ...
            foreach (var pokeStop in pokeStops)
            {
                ...
                var fortInfo = await this._client.GetFort(pokeStop.Id, pokeStop.Latitude, pokeStop.Longitude);
                if (fortInfo == null)
                {
                    continue;
                }
                ...
                var fortSearch = await this._client.SearchFort(pokeStop.Id, pokeStop.Latitude, pokeStop.Longitude);

                this.count++;

                if (fortSearch.ExperienceAwarded > 0)
                {
                    this.failed_softban = 0;
        ...
        {
            var profil = await this._client.GetCachedProfile();
            var stats = await this._inventory.GetPlayerStats();
            ...
            Logger.ColoredConsoleWrite(ConsoleColor.Cyan, "Pokemons: " + await this._inventory.getPokemonCount() + "/" + profil.Profile.PokeStorage);
            Logger.ColoredConsoleWrite(ConsoleColor.Cyan, "Items: " + await this._inventory.getInventoryCount() + "/" + profil.Profile.ItemStorage);
            Logger.ColoredConsoleWrite(ConsoleColor.Cyan, "_____________________________");
        ...
        private async Task TransferDuplicatePokemon(bool keepPokemonsThatCanEvolve = false)
        {
            ...
            {
                var duplicatePokemons = await this._inventory.GetDuplicatePokemonToTransfer(keepPokemonsThatCanEvolve);
